...
import com.thalossphere.agent.plugin.rocketmq.consumer.condition.CidSplitMatchGrayCondition;
    ...
    public List<MessageQueue> allocate(String consumerGroup, String currentCid, List<MessageQueue> mqAll, List<String> cidAll) {
        ...
        if (!isGrayCid) {
            if (log.isDebugEnabled()) {
                log.debug("[thalossphere] gray not-consumer grayConsumerConfig={}", grayConsumerConfig);
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("[thalossphere] gray consumer grayConsumerConfig={}", grayConsumerConfig);
            }
            //当前Cid是灰度节点
