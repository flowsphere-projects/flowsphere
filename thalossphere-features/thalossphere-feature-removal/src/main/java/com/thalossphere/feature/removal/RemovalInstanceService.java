...
import com.thalossphere.feature.discovery.binder.InstanceService;
...
public class RemovalInstanceService {
    ...
    public List<Server> removal(List<Server> instanceList) {
        ...
        if (log.isDebugEnabled()) {
            log.info("[thalossphere] getCanRemovalNum canRemovalNum={} removalSize={}", canRemovalNum, removalInstanceList.size());
        }
        if (canRemovalNum <= 0) {
            return instanceList;
        }
    ...
    private List<Server> removal(List<Server> instanceList, Map<String, ServiceNode> instanceCallResult, RemovalConfig removalConfig,
        ...
        if (log.isDebugEnabled()) {
            log.info("[thalossphere] removal normalInstance={} removalInstance={}", result, instanceCallResult);
        }
        return result;
    }
